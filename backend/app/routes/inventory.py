...
async def create_inventory(payload: InventoryCreate, db: AsyncSession = Depends(get_db)):
    ...
    await log_audit(db, "create", "inventory", inv.id, 1)
    return inv


@router.put("/{inventory_id}", response_model=InventoryResponse)
async def update_inventory(inventory_id: int, payload: InventoryUpdate, db: AsyncSession = Depends(get_db)):
    ...
    await db.commit()
    await db.refresh(inv)
    await log_audit(db, "update", "inventory", inv.id, 1)
    return inv
