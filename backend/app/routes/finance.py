...
async def list_finance(db: AsyncSession = Depends(get_db)):
...
async def create_finance(payload: FinanceCreate, db: AsyncSession = Depends(get_db)):
    finance = Finance(**payload.dict())
    db.add(finance)
    await db.commit()
    await db.refresh(finance)
    await log_audit(db, "create", "finance", finance.id, 1)
    return finance


@router.put("/{finance_id}", response_model=FinanceResponse)
async def update_finance(finance_id: int, payload: FinanceUpdate, db: AsyncSession = Depends(get_db)):
    ...
    await db.commit()
    await db.refresh(finance)
    await log_audit(db, "update", "finance", finance.id, 1)
    return finance
